...
def get_data_list(data_path, ratio=0.1, data_ratio=None):
    img_list = []
    for root, dirs, files in os.walk(data_path):
        if files == []:
            ...
            for f in files:
                ...
                temp_img_list.append(img_path)
            temp_size = len(temp_img_list)
            if data_ratio != None:
                np.random.seed(1)
                temp_img_list = list(np.random.choice(temp_img_list, size=int(temp_size*data_ratio), replace=False))
                print(root, temp_size, len(temp_img_list))
...
def eval_epoch(eval_loader, model, loss_fn, device, best, model_name, loss_name, data_size):
    with torch.no_grad():
        model.eval()
        val_loss = 0
    ...
    if best > val_loss/(batch_idx+1):
        ...
        if torch.cuda.device_count() > 1:
            torch.save(model.module.state_dict(), '/data/temp/yangpengshuai/PIR_KIMIA_model/train_data/{}_{}_{}_{:.4f}.pth'.format(data_size, model_name, loss_name, best))
        else:
            torch.save(model.state_dict(), '/data/temp/yangpengshuai/PIR_KIMIA_model/train_data/{}_{}_{}_{:.4f}.pth'.format(data_size, model_name, loss_name, best))
    return best 
...
if __name__ == '__main__':
    ...
    data_path = '/data/temp/yangpengshuai/PIR_KIMIA_data/train/'
    ...
    for d_ratio in data_ratios:
        ...
        #========================== single data ================================
        class_name, train_img_list, eval_img_list = get_data_list(data_path, data_ratio=d_ratio)
        ...
        for epoch in range(epochs):
            ...
            best = eval_epoch(eval_dataloader, model, loss_fn, device, best, model_name, loss_name, d_ratio)
